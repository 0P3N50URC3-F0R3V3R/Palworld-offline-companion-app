(function () {
    ...
    for (const cat of categories) {
      ...
      for (const e of expeditions.filter(x => x.category === cat)) {
        ...
        html += '<div class="expedition-meta">' +
          ...
          '<span>' + I18N.t('expeditions.fp_prefix') + esc(e.firepower || '') + '</span>' +
        ...
        html += '<div class="expedition-rewards"><div class="section-title">' + I18N.t('encyc.rewards_title') + '</div>';
        for (const r of e.rewards) {
          html += '<div class="reward-row">' +
            ...
            '<span class="reward-qty">' + I18N.t('encyc.qty_prefix') + esc(r.qty) + '</span>' +
  ...
  I18N.ready.then(() => fetch('data/expeditions.json')).then(r => r.json()).then(render).catch(err => {
    document.getElementById('expeditionsList').innerHTML = '<div class="empty">' + I18N.t('encyc.failed_load_expeditions') + '</div>';
    console.error(err);
  });
})();
